...
class HierarchicalLDA(object):
    ...
    def __init__(self, corpus, ratings, brands, vocab, brand_num, brand_idx,
                 alpha=1.0, gamma=1.0, eta=0.1, rho_square = 0.5,
        ...
        self.corpus = corpus
        # regression predictions for each review
        self.w_d = np.zeros(len(corpus))
    ...
    def estimate(self, num_iters, display_topics=50, n_words=5, with_weights=False, fix_tree=False):
        ...
        for iteration in range(num_iters):
            ...
            by_brand = True
            logging.info('iteration ' + str(iteration+1))
    ...
    def sample_existing_path(self, d, s, by_brand):
        ...
        for n in range(len(doc)):  # for each word in the doc
            ...
            #assert level_node.word_weighted_counts[w] >= 0
            #assert level_node.total_weighted_counts >= 0
    ...
    def calculate_likelihood_onlyleaf(self, d, leaf_weights, node, parent_weight, level_word_counts, level, w_0):
        ...
        if node.is_leaf() and node in leaf_weights.keys():
            ...
            w_sum = w_sum/(self.document_length[d])
            # rating likelihood
            leaf_weights[node] += (-(self.ratings[d] - w_sum)** 2 / (2 * self.rho_square))
    ...
    def coherence(self, n_words):
        ...
        cm0 = CoherenceModel(topics=topics_top_words0,
                             dictionary=self.vocab, corpus=bow_corpus, coherence='u_mass')
        score0 = cm0.get_coherence()
        cm1 = CoherenceModel(topics=topics_top_words1,
                             dictionary=self.vocab, corpus=bow_corpus, coherence='u_mass')
        score1 = cm1.get_coherence()
        cm2 = CoherenceModel(topics=topics_top_words2,
                             dictionary=self.vocab, corpus=bow_corpus, coherence='u_mass')
        score2 = cm2.get_coherence()
        cm = CoherenceModel(topics=topics_top_words0+topics_top_words1+topics_top_words2,
                            dictionary=self.vocab, corpus=bow_corpus, coherence='u_mass')
    ...
    def Affinity(self):
        ...
        for topic in self.search_topics(self.root_node):
            if topic.level == 1 and topic.customers > 1000:
                topics1.append(topic)
            if topic.level == 2 and topic.customers > 1000:
        ...
        for t1 in topics1:
            for t2 in topics2:
                if t2 in t1.children:
                    score = cosine_similarity(t1.word_counts.reshape(
                        1, -1), t2.word_counts.reshape(1, -1))[0][0]
        ...
        print(np.mean(cosine_child)/np.mean(cosine_nonchild))
        return
...
if __name__ == "__main__":
    ...
    #data_dir = './data/laptop_5year.txt'   
    ...
    '''
    appearance = [float(content.split('\t')[2]) for content in raw_reviews]
    aroma = [float(content.split('\t')[3]) for content in raw_reviews]
    palate = [float(content.split('\t')[4]) for content in raw_reviews]
    taste = [float(content.split('\t')[5]) for content in raw_reviews]
    '''
    brands = [int(content.split('\t')[6])-1 for content in raw_reviews]
    ...
    for i in range(len(reviews)):
        ...
        sentences = []
        plain_text = []
        for doc in nltk.sent_tokenize(reviews[i]):
            # strip punctuations and digits
            doc = doc.translate(replace)
            doc = doc.encode("utf8").decode("utf8").encode(
                'ascii', 'ignore').decode()  # ignore fancy unicode chars
    ...
    for i in range(len(docs)):
        ...
        for sentence in docs[i]:
            words = [dictionary.token2id[word]
                     for word in sentence if word in dictionary.token2id.keys()]
            if len(words) > 0:
                doc.append(words)
        if len(doc) > 0:
            corpus.append(doc)
    ...
    gamma = 0.005         # nCRP smoothing parameter
    eta = 0.1             # smoothing over topic-word distributions
    num_levels = 3        # the number of levels in the tree
    brand_num = 26        # the number of brands

    model = HierarchicalLDA(corpus=corpus, ratings=corpus_ratings, brands=corpus_brands, vocab=dictionary,
                   brand_num=brand_num, brand_idx=brand_idx, alpha=alpha, gamma=gamma, eta=eta, num_levels=num_levels)
    model.init_assign()
    model.estimate(num_iters=100, display_topics=10, n_words=10)
    
    # save model for further use      
    save_zipped_pickle(model, './models/MHSTM_beer.p')   
    
    print(model.coherence(10))    
    model.Affinity()
